...
class HistoricKlineCache:
    ...
    def get_historical_klines(
        self, ticker_symbol: str, start_date: datetime, end_date: datetime, delta=1
    ):
        data = []
    ...
    def get_historical_ticker_price(
        ...
        target_date = date.replace(second=0, microsecond=0).strftime(
            "%d %b %Y %H:%M:%S"
        )
        key = f"{ticker_symbol} - {target_date}"
        val = targetCache.get(key, None)
        # self.logger.info(f"Fetched prices for {ticker_symbol} at {date} val {val}")
        ...
        if val is None or val == "Missing":
            ...
            if date >= last_day or end_date >= last_day:
                try:
                    data = self.client.get_historical_klines(
                        ticker_symbol, time_frame, target_date, end_date_str, limit=1000
                    )
                    self.logger.info(f"Fetching prices for {ticker_symbol} data {data}")
                    for kline in data:
                        kl_date = datetime.utcfromtimestamp(kline[0] / 1000)
                        kl_datestr = kl_date.strftime("%d %b %Y %H:%M:%S")
                        kl_price = float(kline[4])
                        targetCache[f"{ticker_symbol} - {kl_datestr}"] = kl_price
                except BinanceAPIException as e:
                    if e.code == -1121:  # invalid symbol
                        self.get_historical_klines_from_api(
                            ticker_symbol,
                            time_frame,
                            target_date,
                            end_date_str,
            ...
            val = targetCache.get(key, None)
            self.logger.info(f"Fetching prices for {ticker_symbol} at {date} val {val}")
            if val == None:
    ...
    def get_historical_klines_from_api(
        self,
        ticker_symbol="ETCUSDT",
        interval="1m",
        ...
        if "Contents" not in data["ListBucketResult"]:
            return []
        links = [
            f"https://data.binance.vision/data/spot/{frame}/klines/{ticker_symbol}/{interval}/{ticker_symbol}-{interval}-{fromdate.strftime('%Y-%m-%d')}.zip"
        ]
        ...
        #     if filedate.date().day == fromdate.date().day and filedate.date().month == fromdate.date().month and filedate.date().year == fromdate.date().year:
        ...
        # https://data.binance.vision/data/spot/daily/klines/BTCUSDT/1d/BTCUSDT-1d-2024-05-05.zip

        while len(links) >= 1:
            with ProcessPool() as pool:
                self.logger.info(
                    f"ProcessPool started for {ticker_symbol} between {target_date} and {end_date} links {links}"
                )
