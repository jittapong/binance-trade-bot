...
class TAStrategy(AutoTrader):
    def initialize(self):
        ...
        if self.config_time_frame == "1m":
            self.multiplier = 1
        elif self.config_time_frame == "1h":
            self.multiplier = 60
        elif self.config_time_frame == "4h":
            self.multiplier = 60 * 4
        elif self.config_time_frame == "1d":
        ...
        for coin in self.target_coins:
            ...
            if signal == "buy" and target_coin_balance <= min_qty:
                if bridge_coin_balance >= buy_amount and buy_amount >= min_notional:
                    self.logger.info(f"{current_date.strftime("%Y-%m-%d %H")} >> {coin.symbol}, Signal: {signal}, Price: {current_price}, {signal_info}")
                    self.buy(coin, buy_amount)
                    # self.logger.info(f"{current_date} >> current balances: {self.manager.get_balances(self.all_coins)}\n")
                else:
                    self.logger.info(f"{current_date.strftime("%Y-%m-%d %H")} >> {coin.symbol}, Signal: {signal}, 'Not Enought Money!!\n")
    ...
    def buy(self, coin: Coin, buy_amount):
        ...
        buy_price = self.manager.get_buy_price(coin + self.config.BRIDGE)
        result = self.manager.buy_alt(coin, self.config.BRIDGE, buy_price, buy_quantity)
    ...
    def get_prev_prices_in_range(self, pair_symbol, start_date, end_date, range):
        prev_prices_raw = self.manager.get_ticker_price_in_range(pair_symbol, start_date, end_date, self.multiplier, self.config_time_frame)
        # self.logger.info(f"CDCV3 > start_date: {start_date}, end_date: {end_date}, prev_prices_raw: {prev_prices_raw}")
        if prev_prices_raw is None or len(prev_prices_raw) == 0:
            return None, None
        
        return prev_prices_raw
